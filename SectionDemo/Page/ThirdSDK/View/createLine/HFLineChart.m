...
//    CGFloat chartCavanHeight = self.frame.size.height - UULabelHeight* 3;
    CGFloat levelHeight = 45;//chartCavanHeight / 4.0;
    /*
    //写纵坐标名称
    for (int i = 0; i < 5; i++) {
    ...
    */
    //画横线
    NSInteger horizonLineCount = self.frame.size.height / 45 - 1;
    for (int i = 1; i < horizonLineCount + 1; i++) {
...
- (void)setXLabels:(NSArray *)xLabels
{
    ...
    for (int i=0; i<xLabels.count; i++) {
        ...
        UILabel * label = [self getChartLineLabel];
        label.frame = CGRectMake(i * _xLabelWidth + leftPadding, self.frame.size.height - 45, _xLabelWidth, 45);
